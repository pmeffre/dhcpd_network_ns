...
# argument to command line is VRF
if [ $# -eq 2 -a -n "$2" ]; then
    # skip wait-for-interface behavior if found in path
    if ! which "$2" >/dev/null; then
        # loop until interface is found, or we give up
        NEXT_WAIT_TIME=1
        until [ -e "/var/run/netns/$2" ] || [ $NEXT_WAIT_TIME -eq 4 ]; do
            sleep $(( NEXT_WAIT_TIME++ ))
            echo "Waiting for VRF namespace '$2' to become available... ${NEXT_WAIT_TIME}"
        done
        if [ -e "/var/run/netns/$2" ]; then
            VRF="$2"
	    run="exec ip netns exec $VRF ${run//exec /}"
        fi
    fi
    # first of twice arguments to command line is interface name
    if [ $# -eq 2 -a -n "$1" ]; then
	# skip wait-for-interface behavior if found in path
	if ! which "$1" >/dev/null; then
		# loop until interface is found, or we give up
		NEXT_WAIT_TIME=1
		until [[ $(/sbin/ip netns exec $2 ls /sys/class/net) = *$1* ]] || [ $NEXT_WAIT_TIME -eq 4 ]; do
			sleep $(( NEXT_WAIT_TIME++ ))
			echo "Waiting for interface '$1' to become available... ${NEXT_WAIT_TIME}"
		done
		if [[ $(/sbin/ip netns exec $2 ls /sys/class/net) = *$1* ]]; then
	        	IFACE="$1"
        	fi
	fi
    fi
